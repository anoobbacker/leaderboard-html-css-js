...
! function (e, t) {
	...
	function i(m) {
		...
		function l() {
			if (d && o && (g("Delimiter", "UndetectableDelimiter", "Unable to auto-detect delimiting character; defaulted to '" + v.DefaultDelimiter + "'"), o = !1), m.skipEmptyLines)
				for (var e = 0; e < d.data.length; e++) 1 === d.data[e].length && "" === d.data[e][0] && d.data.splice(e--, 1);
			return p() && function () {
					if (!d) return;
		...
		this.parse = function (e, t, i) {
			if (m.newline || (m.newline = function (e) {
					...
					return r >= t.length / 2 ? "\r\n" : "\r"
				}(e)), o = !1, m.delimiter) M(m.delimiter) && (m.delimiter = m.delimiter(e), d.meta.delimiter = m.delimiter);
			else {
		...
		this.parse = function (n, t, i) {
			...
			for (var p, _ = n.indexOf(x, L), m = n.indexOf(T, L), g = new RegExp(A.replace(/[-[\]/{}()*+?.\\^$|]/g, "\\$&") + S, "g");;)
				if (n[L] !== S)
					...
					} else if (-1 !== _ && (_ < m || -1 === m)) f.push(n.substring(L, _)), L = _ + e, _ = n.indexOf(x, L);
			else {
				if (-1 === m) break;
